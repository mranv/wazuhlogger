...
void req_init()
{
    ...
    {
        mwarn("anubhav, The internal option guess_agent_group must be configured on the master node.");
    }

    // Create hash table
    if (req_table = OSHash_Create(), !req_table)
    {
        merror_exit("anubhav, At OSHash_Create()");
    }
...
void req_sender(int peer, char *buffer, ssize_t length)
{
    ...
    {
    case 0:
        merror("anubhav, At OSHash_Add()");
        req_free(node);
        break;

    case 1:
        merror("anubhav, At OSHash_Add(): Duplicated counter.");
        req_free(node);
        break;
    ...
    case 2:
        ...
        if (!req_pool_wait())
        {
            merror("anubhav, Request pool wait failed.");
            break;
        }

        // Run thread
        mdebug2("anubhav, Creating dispatcher thread for counter %s", counter_s);
    ...
    merror("anubhav, Sending internal error to peer.");
...
void *req_dispatch(req_node_t *node)
{
    ...
    mdebug2("anubhav, Running request dispatcher thread. Counter=%s", node->counter);
    ...
    {
        merror("anubhav, Request has no agent id.");
        goto cleanup;
    }
    ...
    *_payload = '\0';
    ...
    mdebug2("anubhav, Sending request: '%s'", payload);
    ...
    if (protocol < 0)
    {
        merror("anubhav, No protocol found for agent '%s'", agentid);
        goto cleanup;
    }
    ...
    for (attempts = 0; attempts < max_attempts; attempts++)
    {
        ...
        {
            merror("anubhav, Cannot send request to agent '%s'", agentid);
        ...
        mdebug2("anubhav, Timeout for waiting ACK from agent '%s', resending.", agentid);
    }

    if (attempts == max_attempts)
    {
        merror("anubhav, Couldn't send request to agent '%s': number of attempts exceeded.", agentid);
    ...
    for (attempts = 0; attempts < max_attempts && (!node->buffer || IS_ACK(node->buffer)); attempts++)
    {
        ...
        {
            merror("anubhav, Response timeout for request counter '%s'", node->counter);
    ...
    if (attempts == max_attempts)
    {
        merror("anubhav, Couldn't get response from agent '%s': number of attempts exceeded.", agentid);
        OS_SendSecureTCP(node->sock, strlen(WR_ATTEMPT_ERROR), WR_ATTEMPT_ERROR);
        goto cleanup;
    }
    ...
    if (protocol == REMOTED_NET_PROTOCOL_UDP)
    {
        mdebug2("anubhav, Sending ack (%s).", node->counter);
    ...
    if (node->buffer)
    {
        mdebug2("anubhav, Sending response: '%s'", node->buffer);
    }

    if (OS_SendSecureTCP(node->sock, node->length, node->buffer) != 0)
    {
        mwarn("anubhav, At OS_SendSecureTCP(): %s", strerror(errno));
    }
...
cleanup:
    ...
    {
        merror("anubhav, At OSHash_Delete(): no such key.");
    }
...
int req_save(const char *counter, const char *buffer, size_t length)
{
    ...
    mdebug2("anubhav, Saving '%s:%s'", counter, buffer);
    ...
    {
        mdebug1("anubhav, Request counter (%s) not found. Duplicated message?", counter);
        retval = -1;
    }
...
int req_pool_wait()
{
    ...
    while (!request_pool && wait_ok)
    {
        ...
        case ETIMEDOUT:
            merror("anubhav, Request pool is full. Rejecting request.");
            wait_ok = 0;
            break;

        default:
            merror("anubhav, At w_cond_timedwait(): %s", strerror(errno));
            wait_ok = 0;
            break;
        }
    }
