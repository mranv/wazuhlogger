...
void save_controlmsg(const keyentry *key, char *r_msg, size_t msg_length, int *wdb_sock)
{
    ...
    mdebug2("anubhav, Processing control message from agent '%s' with ID '%s'.", key->name, key->id);
    ...
    if (strncmp(r_msg, HC_REQUEST, strlen(HC_REQUEST)) == 0)
    {
        ...
        if ((payload = strchr(counter, ' ')) == NULL)
        {
            merror("anubhav, Request control format error. r_msg: \"%s\"", r_msg);
            return;
        }

        *(payload++) = '\0';

        if (req_save(counter, payload, msg_length - (payload - r_msg)) != 0)
        {
            merror("anubhav, Failed to save request with counter '%s'.", counter);
        }
    ...
    if (strncmp(r_msg, HC_STARTUP, strlen(HC_STARTUP)) == 0 || strcmp(r_msg, HC_SHUTDOWN) == 0)
    {
        ...
        {
            mdebug1("anubhav, Agent %s sent HC_STARTUP from '%s'.", key->name, aux_ip);
            cJSON *agent_info = NULL;
            if ((agent_info = cJSON_Parse(strchr(r_msg, '{'))) != NULL)
            {
                cJSON *version = NULL;
                if ((version = cJSON_GetObjectItem(agent_info, "version")) != NULL && cJSON_IsString(version))
                {
                ...
                {
                    merror("anubhav, Error getting version from agent '%s'", key->id);
        ...
        {
            mdebug1("anubhav, Agent %s sent HC_SHUTDOWN from '%s'.", key->name, aux_ip);
            is_shutdown = 1;
    ...
    else
    {
        ...
        {
            mwarn("anubhav, Invalid message from agent: '%s' (%s)", key->name, key->id);
            os_free(clean);
            return;
        }
    ...
    if (is_shutdown == 0)
    {
        ...
        {
            merror("anubhav, Failed to send ACK message to agent '%s'", key->id);
        }
    }
    ...
    if ((data = OSHash_Get(pending_data, key->id)) != NULL && data->changed && data->message && msg && strcmp(data->message, msg) == 0)
    {
        ...
        {
            mwarn("anubhav, Unable to save last keepalive and set connection status as active for agent: %s", key->id);
        }
    }
    else
    {
        if (data == NULL)
        {
            ...
            {
                merror("anubhav, Couldn't add pending data into hash table for agent: %s", key->id);
                w_mutex_unlock(&lastmsg_mutex);
                os_free(data);
                os_free(clean);
                return;
        ...
        if (is_startup)
        {
            ...
            if (OS_SUCCESS != result)
            {
                mwarn("anubhav, Unable to save last keepalive and set connection status as pending for agent: %s", key->id);
            }
        }
        else if (is_shutdown)
        {
            ...
            if (OS_SUCCESS != result)
            {
                mwarn("anubhav, Unable to set connection status as disconnected for agent: %s", key->id);
            }
            else
            {
                /* Generate alert */
                ...
                {
                    merror("anubhav, Failed to send stopped message to queue '%s': %s", DEFAULTQUEUE, strerror(errno));

                    // Try to reconnect infinitely
                    logr.m_queue = StartMQ(DEFAULTQUEUE, WRITE, INFINITE_OPENQ_ATTEMPTS);

                    minfo("anubhav, Successfully reconnected to '%s'", DEFAULTQUEUE);

                    if (SendMSG(logr.m_queue, msg, srcmsg, SECURE_MQ) < 0)
                    {
                        // Something went wrong sending a message after an immediate reconnection...
                        merror("anubhav, Failed to send stopped message to queue '%s' after reconnection: %s", DEFAULTQUEUE, strerror(errno));
                    }
                }
            }
        }
        ...
        {
            /* Update message */
            mdebug2("anubhav, save_controlmsg(): inserting '%s' for agent '%s'", msg, key->id);
            ...
            if (OS_SUCCESS == lookfor_agent_group(key->id, data->message, &data->group, wdb_sock))
            {
                ...
                if ((aux = OSHash_Get_ex(groups, data->group)) == NULL)
                {
                    if ((aux = OSHash_Get_ex(multi_groups, data->group)) == NULL)
                    {
                        mdebug1("anubhav, No such group '%s' for agent '%s'", data->group, key->id);
                    }
                }
            ...
            {
                merror("anubhav, Error getting group for agent '%s'", key->id);
            }
            ...
            if (OS_SUCCESS != result)
            {
                merror("anubhav, Error parsing message for agent '%s'", key->id);
            ...
            {
                mwarn("anubhav, Unable to get hostname due to: '%s'", strerror(errno));
            }
            else
            {
            ...
            {
                mdebug1("anubhav, Unable to update information in global.db for agent: %s", key->id);
            }
...
cJSON *assign_group_to_agent(const char *agent_id, const char *md5)
{
    ...
    mdebug2("anubhav, Assigning group to agent '%s' with file '%s' and MD5 '%s'.", agent_id, SHAREDCFG_FILENAME, md5);
    ...
    if (!guess_agent_group || (!find_group_from_sum(md5, group) && !find_multi_group_from_sum(md5, group)))
    {
        ...
        mdebug2("anubhav, No group found for MD5 '%s'. Setting to default group.", md5);
    }

    w_mutex_unlock(&files_mutex);

    // Update the agent's group in the database
    if (wdb_set_agent_groups_csv(atoi(agent_id),
                                 group,
                                 WDB_GROUP_MODE_EMPTY_ONLY,
                                 w_is_single_node(NULL) ? "synced" : "syncreq",
                                 NULL) != OS_SUCCESS)
    {
        merror("anubhav, Failed to update agent '%s' group in database.", agent_id);
    }
    else
    {
        mdebug2("anubhav, Group successfully assigned for agent '%s'.", agent_id);
    }
...
STATIC cJSON *assign_group_to_agent_worker(const char *agent_id, const char *md5)
{
    ...
    mdebug2("anubhav, Sending message to master node: '%s'", request);

    // Send request to the master node and receive the response
    if (w_send_clustered_message("sendsync", request, response) == 0)
    {
        ...
        if (response_json)
        {
            cJSON *data_item = cJSON_GetObjectItem(response_json, "data");
            if (data_item)
            {
                data_json = cJSON_Duplicate(data_item, 1);
            }
            else
            {
                merror("anubhav, No 'data' item in response from master node.");
            }
            cJSON_Delete(response_json);
        }
        else
        {
            merror("anubhav, Failed to parse response JSON from master node.");
        }
    }
    else
    {
        merror("anubhav, Failed to send message to master node.");
    }

    mdebug2("anubhav, Message received from master node: '%s'", response);
...
STATIC void c_group(const char *group, OSHash **_f_time, os_md5 *_merged_sum, char *sharedcfg_dir, bool create_merged, bool is_multigroup)
{
    ...
    if (create_merged && (r_group = w_parser_get_group(group), r_group))
    {
        if (r_group->current_polling_time <= 0)
        {
            ...
            if (r_group->merge_file_index >= 0)
            {
                ...
                mdebug1("anubhav, Downloading shared file '%s' from '%s'", merged, file_url);
                ...
                if (r_group->merged_is_downloaded)
                {
                    ...
                    if (!TestUnmergeFiles(destination_path, OS_TEXT))
                    {
                        ...
                        merror("anubhav, The downloaded file '%s' is corrupted.", destination_path);

                        if (fd == -1)
                        {
                            merror("anubhav, Failed to delete file '%s'", destination_path);
                        }
                        return;
                    }
            ...
            { // Download all files
                ...
                if (r_group->files)
                {
                    for (i = 0; r_group->files[i].name; i++)
                    {
                        ...
                        mdebug1("anubhav, Downloading shared file '%s' from '%s'", destination_path, file_url);
    ...
    if ((!r_group || !r_group->merged_is_downloaded) && (!is_multigroup || create_merged))
    {
        if (create_merged)
        {
            if (disk_storage)
            {
                ...
                {
                    merror("anubhav, Unable to create merged file: '%s' due to [(%d)-(%s)].", merged_tmp, errno, strerror(errno));
                    return;
                }
            }
            else
            {
                ...
                {
                    merror("anubhav, Unable to open memory stream due to [(%d)-(%s)].", errno, strerror(errno));
                    os_free(finalbuf);
                    return;
                }
            }
            fprintf(finalfp, "#%s\n", group);
            mdebug1("anubhav, Created merged file stream: '%s'", merged_tmp);
        }
        ...
        if (stat(DEFAULTAR, &attrib) == 0)
        {
            if (create_merged)
            {
                if (merged_ok = MergeAppendFile(finalfp, DEFAULTAR, -1), merged_ok == 0)
                {
                    fclose(finalfp);
                    os_free(finalbuf);
                    merror("anubhav, MergeAppendFile failed for '%s'", DEFAULTAR);
                    return;
                }
            }
            if (!is_multigroup)
        ...
        if (create_merged)
        {
            if (merged_ok == 0)
            {
                fclose(finalfp);
                os_free(finalbuf);
                merror("anubhav, validate_shared_files failed for group path '%s'", group_path);
                return;
            }
        }
        ...
        if (create_merged)
        {
            ...
            if (disk_storage)
            {
                ...
                {
                    merror("anubhav, Accessing file '%s'", merged_tmp);
                    return;
                }
            }
            else
            ...
            if ((OS_MD5_File(merged, md5sum, OS_TEXT) != 0) || (strcmp(md5sum_tmp, md5sum) != 0))
            {
                if (disk_storage)
                {
                    ...
                    mdebug1("anubhav, Moved file '%s' to '%s'", merged_tmp, merged);
                }
                else
                {
                    if (finalfp = fopen(merged, "w"), finalfp == NULL)
                    {
                        merror("anubhav, Unable to open file: '%s' due to [(%d)-(%s)].", merged, errno, strerror(errno));
                        os_free(finalbuf);
                        return;
                    }
                    fwrite(finalbuf, finalsize, 1, finalfp);
                    fclose(finalfp);
                    mdebug1("anubhav, Wrote final buffer to file '%s'", merged);
                    os_free(finalbuf);
                }
            }
            else
            {
                ...
                {
                    unlink(merged_tmp);
                    mdebug1("anubhav, Unlinked temporary file '%s'", merged_tmp);
                }
                else
                {
                    os_free(finalbuf);
    ...
    if (OS_MD5_File(merged, md5sum, OS_TEXT) == 0)
    {
        ...
        {
            merror("anubhav, Unable to get entry attributes '%s'", merged);
        }
        else
        {
    ...
    {
        merror("anubhav, Accessing file '%s'", merged);
    }
}
...
STATIC void c_files(bool initial_scan)
{
    mdebug2("anubhav, Updating shared files.");

    w_mutex_lock(&files_mutex);
    mdebug2("anubhav, Acquired files mutex.");

    /* Analyze groups */
    mdebug2("anubhav, Analyzing groups.");
    process_groups();
    mdebug2("anubhav, Groups analysis completed.");

    /* Analyze multigroups */
    mdebug2("anubhav, Analyzing multigroups.");
    process_multi_groups();
    mdebug2("anubhav, Multigroups analysis completed.");

    /* Delete residual groups */
    mdebug2("anubhav, Deleting residual groups.");
    process_deleted_groups();
    mdebug2("anubhav, Residual groups deletion completed.");

    /* Delete residual multigroups */
    mdebug2("anubhav, Deleting residual multigroups.");
    process_deleted_multi_groups(initial_scan);
    mdebug2("anubhav, Residual multigroups deletion completed.");

    w_mutex_unlock(&files_mutex);
    mdebug2("anubhav, Released files mutex.");

    if (!reported_path_size_exceeded)
    {
        reported_path_size_exceeded = 1;
        mdebug2("anubhav, Reported path size exceeded flag set.");
    }

    mdebug2("anubhav, End updating shared files.");
}
...
STATIC int validate_shared_files(const char *src_path, FILE *finalfp, OSHash **_f_time, bool create_merged, bool is_multigroup, int path_offset)
{
    ...
    // Try to open directory, avoid TOCTOU hazard
    if ((files = wreaddir(src_path)) == NULL)
    {
        if (errno != ENOTDIR)
        {
            mdebug1("anubhav, Could not open directory '%s'", src_path);
        }
        return 1;
    }

    mdebug2("anubhav, Reading directory '%s'", src_path);
    ...
    for (i = 0; files[i]; ++i)
    {
        ...
        if (snprintf(file, MAX_SHARED_PATH + 1, "%s/%s", src_path, files[i]) > MAX_SHARED_PATH)
        {
            if (!reported_path_size_exceeded)
            {
                mwarn("anubhav, Path too long '%s'", file);
            }
            else
            {
                mdebug2("anubhav, Path too long '%s'", file);
            }
            continue;
        }
        ...
        {
            merror("anubhav, Unable to get entry attributes '%s'", file);
            continue;
        }

        if (S_ISDIR(attrib.st_mode))
        {
            mdebug2("anubhav, Directory found: '%s'", file);
            if ((merged_ok = validate_shared_files(file, finalfp, _f_time, create_merged, is_multigroup, path_offset)) == 0)
            {
                free_strarray(files);
                return 0;
            }
        ...
        {
            // Is a file
            if ((modify_time = (time_t *)OSHash_Get(invalid_files, file)) != NULL)
            {
                time_t last_modify;
                ...
                if (*modify_time != last_modify)
                {
                    *modify_time = last_modify;
                    if (checkBinaryFile(file))
                    {
                        ...
                        mdebug1("anubhav, File '%s' modified but still invalid.", file);
                    }
                    else
                    {
                        os_free(modify_time);
                        OSHash_Delete(invalid_files, file);
                        minfo("anubhav, File '%s' is valid after last modification.", file);
                        ignored = 0;
                    }
                }
            }
            else
            {
                if (checkBinaryFile(file))
                {
                    ...
                    *modify_time = attrib.st_mtime;
                    ...
                    if ((ret_val = OSHash_Add(invalid_files, file, modify_time)) != 2)
                    {
                        os_free(modify_time);
                        if (ret_val == 0)
                        {
                            merror("anubhav, Unable to add file '%s' to hash table of invalid files.", file);
                        }
                    }
                    else
                    {
                        merror("anubhav, Invalid shared file '%s'. Ignoring it.", file);
                    }
                }
            }
            ...
            if (!ignored)
            {
                if (create_merged)
                {
                    mdebug2("anubhav, Appending file '%s' to merged file.", file);
                    if ((merged_ok = MergeAppendFile(finalfp, file, path_offset)) == 0)
                    {
                        free_strarray(files);
                        return 0;
                    }
...
static int send_file_toagent(const char *agent_id, const char *group, const char *name, const char *sum, char *sharedcfg_dir)
{
    ...
    mdebug1("anubhav, Entering send_file_toagent function. Parameters: agent_id='%s', group='%s', name='%s', sum='%s', sharedcfg_dir='%s'",
            agent_id, group, name, sum, sharedcfg_dir);
    ...
    if (strchr(group, MULTIGROUP_SEPARATOR))
    {
        ...
        mdebug1("anubhav, Multigroup detected. File path: '%s'", file);
    }
    else
    {
        snprintf(file, OS_SIZE_1024, "%s/%s/%s", sharedcfg_dir, group, name);
        mdebug1("anubhav, Single group. File path: '%s'", file);
    }

    fp = fopen(file, "r");
    if (!fp)
    {
        mdebug1("anubhav, Error opening file '%s': %s", file, strerror(errno));
        return OS_INVALID;
    }
    mdebug1("anubhav, File '%s' opened successfully.", file);

    /* Send the file name first */
    snprintf(buf, OS_SIZE_1024, "%s%s%s %s\n",
             CONTROL_HEADER, FILE_UPDATE_HEADER, sum, name);

    mdebug1("anubhav, Sending file header: '%s'", buf);

    if (send_msg(agent_id, buf, -1) < 0)
    {
        mdebug1("anubhav, Error sending file header to agent '%s'.", agent_id);
        fclose(fp);
        return OS_INVALID;
    }
    else
    {
        ...
        mdebug1("anubhav, File header sent successfully to agent '%s'.", agent_id);
    }
    ...
    if (protocol < 0)
    {
        merror("anubhav, Error retrieving protocol for agent '%s'.", agent_id);
        fclose(fp);
        return OS_INVALID;
    }
    mdebug1("anubhav, Protocol for agent '%s': %d", agent_id, protocol);

    /* Send the file contents */
    while ((n = fread(buf, 1, 900, fp)) > 0)
    {
        buf[n] = '\0';
        mdebug1("anubhav, Sending file chunk: size=%zu", n);

        if (send_msg(agent_id, buf, -1) < 0)
        {
            mdebug1("anubhav, Error sending file chunk to agent '%s'.", agent_id);
            fclose(fp);
            return OS_INVALID;
        }
        else
        {
            rem_inc_send_shared(agent_id);
            mdebug1("anubhav, File chunk sent successfully to agent '%s'.", agent_id);
        }
        ...
        if (protocol == REMOTED_NET_PROTOCOL_UDP)
        {
            ...
            if (i > 30)
            {
                mdebug1("anubhav, Sleeping to avoid UDP flood. Message count: %d", i);
                sleep(1);
                i = 0;
            }
            i++;
    ...
    mdebug1("anubhav, Sending file close message: '%s'", buf);

    if (send_msg(agent_id, buf, -1) < 0)
    {
        mdebug1("anubhav, Error sending file close message to agent '%s'.", agent_id);
        fclose(fp);
        return OS_INVALID;
    }
    else
    {
        rem_inc_send_shared(agent_id);
        mdebug1("anubhav, File close message sent successfully to agent '%s'.", agent_id);
    }

    fclose(fp);
    mdebug1("anubhav, File '%s' closed and function exiting successfully.", file);
...
void *wait_for_msgs(__attribute__((unused)) void *none)
{
    ...
    mdebug1("anubhav, Entering wait_for_msgs function.");
    ...
    while (1)
    {
        ...
        mdebug1("anubhav, Popped agent_id '%s' from queue.", agent_id);
        ...
        if (data = OSHash_Get(pending_data, agent_id), data)
        {
            ...
            mdebug1("anubhav, Retrieved pending data for agent ID '%s': group='%s', merged_sum='...'", agent_id, group);
        }
        else
        {
            merror("anubhav, Couldn't get pending data from hash table for agent ID '%s'.", agent_id);
            os_free(agent_id);
            agent_id = NULL;
        }
        ...
        {
            mdebug1("anubhav, Sending file '%s/%s' to agent '%s'.", group, SHAREDCFG_FILENAME, agent_id);
            ...
            if (multi_group)
            {
                ...
                mdebug1("anubhav, Multi-group detected. Using directory '%s'.", sharedcfg_dir);
            }
            else
            {
                strcpy(sharedcfg_dir, SHAREDCFG_DIR);
                mdebug1("anubhav, Single group detected. Using directory '%s'.", sharedcfg_dir);
            }
            ...
            mdebug2("anubhav, End sending file '%s/%s' to agent '%s'.", group, SHAREDCFG_FILENAME, agent_id);
        }
        else
        {
            mdebug1("anubhav, Skipping sending file. Missing agent_id, group, or merged_sum.");
        }
        ...
        {
            data->changed = 0;
            mdebug1("anubhav, Marked pending data for agent ID '%s' as dispatched.", agent_id);
        }

        w_mutex_unlock(&lastmsg_mutex);
    }

    mdebug1("anubhav, Exiting wait_for_msgs function.");
    return NULL;
}
...
void *update_shared_files(__attribute__((unused)) void *none)
{
    ...
    mdebug1("anubhav, Entering update_shared_files function. Interval set to %d seconds.", INTERVAL);

    time_t _stime = time(0);
    ...
    while (1)
    {
        ...
        {
            mdebug1("anubhav, Checking if YAML file has changed.");

            // Check if the yaml file has changed and reload it
            if (w_yaml_file_has_changed())
            {
                mdebug1("anubhav, YAML file has changed. Reloading file and updating structures.");

                if (w_yaml_file_update_structs() == 0)
                {
                    merror("anubhav, Failed to update YAML file structures.");
                }

                w_yaml_create_groups();
                mdebug1("anubhav, YAML file reloaded and groups created.");
            }
            else
            {
                mdebug1("anubhav, YAML file has not changed.");
            }

            c_files(false);
            _stime = _ctime;
            mdebug1("anubhav, File check and update complete. Waiting for the next interval.");
        }

        sleep(1);
    }

    mdebug1("anubhav, Exiting update_shared_files function.");
    return NULL;
}
